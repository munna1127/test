...
const AccessRequest = require('./models/AccessRequest'); // Naya Model
...
app.post('/check-access', async (req, res) => {
    try {
        const access = await AccessRequest.findOne({ userEmail: req.body.userEmail, examName: req.body.examName, testName: req.body.testName }).sort({ date: -1 });
        res.status(200).json(access || { status: 'none' });
    } catch (e) { res.status(500).json({ error: "Failed" }); }
});

app.post('/request-access', async (req, res) => {
    try {
        let reqRecord = await AccessRequest.findOne({ userEmail: req.body.userEmail, examName: req.body.examName, testName: req.body.testName });
        if (reqRecord) { reqRecord.status = 'pending'; reqRecord.date = Date.now(); } 
        else { reqRecord = new AccessRequest(req.body); }
...
app.post('/leaderboard', async (req, res) => { try { res.status(200).json(await Result.find({ examName: req.body.examName, testName: req.body.testName }).sort({ score: -1, date: 1 })); } catch (e) { res.status(500).json({ error: "Failed" }); } });
